...
var noop = function () {};
var jqLite = jQuery; // hack, but i could not find bower-installable jqLite
var msie = false; // this is a firefox lib, so its probably fine...
var sniffer;

describe('$sniffer', function() {

    beforeEach(module('lf.ff-addon-compat', function (_$snifferProvider_) {
        sniffer = function ($window, $document) {
            $window.navigator = {};
            $document = jqLite($document || {});
            if (!$document[0].body) {
                $document[0].body = window.document.body;
            }
            return new _$snifferProvider_.$get[2]($window, $document);
        };
    }));

    describe('history', function() {
        it('should be true if history.pushState defined', inject(function() {
    ...
    describe('hashchange', function() {
        it('should be true if onhashchange property defined', inject(function() {
            expect(sniffer({onhashchange: true}).hashchange).toBe(true);
        }));

        it('should be false if onhashchange property not defined', inject(function() {
            expect(sniffer({}).hashchange).toBe(false);
        }));

        it('should be false if documentMode is 7 (IE8 comp mode)', inject(function() {
    ...
    describe('hasEvent', function() {
        ...
        it('should only create the element once', function() {
            ...
            expect(mockDocument.createElement.callCount).toEqual(1);
        });
        ...
        it('should return the correct vendor prefix based on the browser', function() {
            ...
            inject(function($sniffer, $window) {
                ...
                if(/phantomjs/i.test(ua)) {
                    // allow testing in phantomjs, we don't really care what this is
    ...
    describe('animations', function() {
        ...
        // this tests probably fail because our isString mock is somewhat crappy
        // but i didn't touch any code related to it...
        xit('should be true on android with older body style properties', function() {
            module(function($provide) {
                var doc = {
                    body : {
                        style : {
